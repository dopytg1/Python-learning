...
class Checker():
    ...
    def check_win(self, char: str):
        ...
        for each in fieldForChecking:
            ...
            confirmAll.append(confirm)
        print(confirmAll)

        if confirm:
            for i in range(len(win_cords) - 3):
                count_list = []
                for each in win_cords[i]:
                    count = 0
                    for i in range(len(each)):
                        if each[i] == char:
                            break
                ...
                win_or_not = 0
                for j in range(len(fieldForChecking)):
                    if fieldForChecking[j][count_list[j]] == char:
                        win_or_not += 1
                    else:
                        continue
            ...
            for j in range(len(win_cords[i])):
                if win_cords[i][j] in fieldForChecking[j]:
                    winOrNot += 1
